...
**Target Beneficiaries (Updated):**
- The platform serves learners at multiple levels: elementary, middle school, high school, TVET/diploma, university (bachelor/master/phd), and other eligible scholarship tracks.
- Scholarship discovery and recommendations should support different scholarship types and level-specific eligibility.

### Role Governance Model (Updated)

The platform uses four roles:

- **ADMIN** - system governance and full platform control
- **OWNER** - business/platform owner, created by admin
- **MANAGER** - created by owner, manages own scholarship portfolio
- **STUDENT** - end user who discovers, bookmarks, and applies

#### Creation Rules
- Admin can create Owner accounts.
- Owner can create Manager accounts.
- Public registration is Student-only.

#### Permission Matrix (Operational Baseline)

| Module / Action | ADMIN | OWNER | MANAGER | STUDENT |
|---|---|---|---|---|
| User management | All users/roles | Managers + scoped users | No | Self only |
| Create scholarships | Yes | Yes | Yes | No |
| Update/Delete scholarships | Any scholarship | Own + owner-scoped manager scholarships | Own only | No |
| Verification (approve/reject) | Yes | No | No | No |
| Dashboard scope | System-wide | Owner scope | Own scope | Personal |
| Applications view | All | Owner scope | Own scholarships | Own only |
...
- student_profiles (user_id, field_of_study, gpa, current_education_level, target_education_levels, preferred_country, interests)
- scholarships (id, title, provider, country, target_education_levels, scholarship_type, field_of_study, funding_type, deadline, status, created_by, verified_by)
...
#### Purpose
Secure system access with role-based permissions (Admin, Owner, Manager, Student)
...
5. Implement role management (promote/demote)
...
- Field of study
- GPA
- Current education level (Elementary, Middle School, High School, TVET/Diploma, Bachelor's, Master's, PhD, Other)
- Target education levels (multi-select)
- Preferred country
- Interests (array)
...
```javascript
{
  ...
  target_education_levels: Enum[] (required),
...
#### API Endpoints
```
GET    /api/scholarships/search?q=engineering&country=USA&education_level=high_school&funding_type=fully_funded&sort=deadline_asc&page=1&limit=20
